...
echo "$($PSQL "SELECT DISTINCT(name) FROM teams FULL JOIN games ON teams.team_id=games.opponent_id OR teams.team_id=games.winner_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"

echo -e "\nList of unique winning team names in the whole data set:"
echo "$($PSQL "SELECT name FROM teams FULL JOIN games ON teams.team_id=games.winner_id WHERE round='Eighth-Final' OR round='Semi-Final' OR round='Quarter-Final' OR round='Final' GROUP BY name ORDER BY name")"
